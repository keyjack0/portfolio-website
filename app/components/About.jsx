    ...
    <div id='about' className='w-full px-[12%] py-20 mt-20 scroll-mt-20'>
      ...
      <div className='flex w-full flex-col lg:flex-row items-center gap-20 my-20'>
        <div className='w-64 sm:w-80 rounded-3xl max-w-none'>
          <Image src={assets.fotodiri} alt='user' className='w-full object-cover h-100 rounded-3xl' />
        </div>
        <div className='flex-1'>
          ...
          <ul className='grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-2xl'>
            {infoList.map(({ icon, iconDark, title, description }, index) => (
              <li className='border-[0.5px] border-gray-400 rounded-xl p-6 cursor-pointer hover:-translate-y-1 duration-500 hover:shadow-black' key={index}>
